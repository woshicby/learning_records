...
myTreeGini = create_tree_gini(myDat, Labels)
print('按香农熵生成的决策树为：', myTree)
print('按基尼不纯度生成的决策树为：', myTreeGini)
print('----------进行存取测试测试----------')
store_tree(myTree, filePath + '/%s' % fileName)
print('存入决策树为：', myTree)
myTree = grab_tree(filePath + '/%s' % fileName)
print('取出决策树为：', myTree)
print('----------进行分类测试----------')
print('[1,0]按香农熵生成的决策树分类结果为：', classify(myTree, Labels, [1, 0]))
print('[1,1]按基尼不纯度生成的决策树分类结果为：', classify(myTreeGini, Labels, [1, 1]))
