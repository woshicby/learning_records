...
def build_stump(data_arr, class_labels, d):  # 生成单层决策树
    ...
    for i in range(n):  # 对每一个特征循环
        range_min = data_matrix[:, i].min()  # 最小值
        range_max = data_matrix[:, i].max()  # 最大值
        ...
        for j in range(-1, int(num_steps) + 1):  # 对每个步长循环
            for unequal_sign in ['小于', '大于']:  # 对每个不等号循环（切换大于小于）
                ...
                predicted_vals = stump_classify(data_matrix, i, thresh_val, unequal_sign)  # 调用stump_class()
                ...
                # print("目前维数：%d，阈值：%.2f，不等号：%s，加权错误率为：%.3f" % (i + 1, thresh_val, unequal_sign, weighted_error))
                if weighted_error < min_error:  # 若加权错误率小于记录的最小值，更新相关数据
                    ...
                    # print('【更新最小值】当前最佳维数为%d，最佳阈值为%.2f，不等号为：%s' % (i + 1, thresh_val, unequal_sign))
...
def ada_boost_train_ds(data_arr, class_labels, num_iteration=40):  # 基于单层决策树的AdaBoost训练过程
    ...
    for i in range(num_iteration):  # 进行num_iteration次迭代
...
def ada_classify(dat_to_class, classifier_arr):  # 分类器
    ...
    for i in range(len(classifier_arr)):
        class_est = stump_classify(data_matrix, classifier_arr[i]['维数'], classifier_arr[i]['阈值'], classifier_arr[i]['不等号'])  # 调用stump_classify()
