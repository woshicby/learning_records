...
# #####设置区域#####
trainFile = r'D:\Desktop\新建文件夹\machinelearninginaction3x-master\Ch09\bikeSpeedVsIq_train.txt'
testFile = r'D:\Desktop\新建文件夹\machinelearninginaction3x-master\Ch09\bikeSpeedVsIq_test.txt'
...
# #####运行区域#####
trainMat = numpy.mat(load_data_set(trainFile))
testMat = numpy.mat(load_data_set(testFile))
myTree = create_tree(trainMat, ops=(1, 20))
yHat = create_fore_cast(myTree, testMat[:, 0])
print(numpy.corrcoef(yHat, testMat[:, 1], rowvar=0)[0, 1])
myTree = create_tree(trainMat, model_leaf, model_err, (1, 20))
yHat = create_fore_cast(myTree, testMat[:, 0], model_tree_eval)
print(numpy.corrcoef(yHat, testMat[:, 1], rowvar=0)[0, 1])
wireSet,X,Y=linear_solve(trainMat)
for i in range(numpy.shape(testMat)[0]):
    yHat[i]=testMat[i,0]*wireSet[1,0]+wireSet[0,0]
print(numpy.corrcoef(yHat, testMat[:, 1], rowvar=0)[0, 1])
